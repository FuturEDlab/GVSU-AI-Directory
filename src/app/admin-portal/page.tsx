...
export default function AdminPortal() {
  ...
  const handleRunVettingAgent = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Agent Failure" });
    } finally {
      setIsVetting(false);
    }
  };
                  ...
                  <header className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-gvsuBlue rounded-2xl flex items-center justify-center text-white font-bold text-xl shadow-lg overflow-hidden">
                        {draftImage ? <img src={draftImage} className="w-full h-full object-cover" /> : draftTitle.substring(0, 1).toUpperCase()}
                      </div>
                      <div>
                      ...
                      {activeTool.reportCard && (
                        <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-6">
                           ...
                           <div className="grid grid-cols-2 gap-6">
                              {Object.entries(activeTool.reportCard).map(([key, data]: any) => (
                                <div key={key} className="space-y-1">
                                  <div className="flex justify-between items-center">
                                    <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400">{key.replace('_', ' ')}</span>
